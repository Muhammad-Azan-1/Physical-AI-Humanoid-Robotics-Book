...
const AuthStateSync: React.FC = () => {
  ...
  useEffect(() => {
    // Set isClient to true on mount to ensure we're in the browser
    setIsClient(true);
  }, []);

  // Only initialize broadcast channel on the client side
  const broadcastChannel = isClient ? getBroadcastChannelManager() : null;

  // Handle authentication state updates from other tabs
  const handleAuthUpdate = useCallback((data: AuthStateData) => {
    if (!isClient || !broadcastChannel) return;
  ...
  }, [isClient, broadcastChannel, setUser, setSession, setIsAuthenticated]);
  ...
  }, [isClient, broadcastChannel, signOut]);
  ...
  }, [isClient, broadcastChannel, handleAuthUpdate, handleLogout, handleTokenExpired, handleTokenRefresh]);
